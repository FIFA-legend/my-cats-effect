...
object IOErrorHandling {
  ...
  def option2IO[A](option: Option[A])(ifEmpty: Throwable): IO[A] = option match {
    case Some(value) => IO(value) // IO.pure(value)
  ...
  def handleIOError[A](io: IO[A])(handler: Throwable => A): IO[A] = io.redeem(handler, a => a)

  def handleIOErrorRTJ[A](io: IO[A])(handler: Throwable => A): IO[A] = io.redeem(handler, identity)

  def handleIOErrorWith[A](io: IO[A])(handler: Throwable => IO[A]): IO[A] = io.redeemWith(handler, IO(_))

  def handleIOErrorWithRTJ[A](io: IO[A])(handler: Throwable => IO[A]): IO[A] = io.redeemWith(handler, IO.pure)
