...
object IOTraversal extends IOApp.Simple {
  ...
  // hard version
  def sequence_v2[F[_] : Traverse, A](container: F[IO[A]]): IO[F[A]] =
    Traverse[F].traverse(container)(x => x)
  ...
  // hard version
  def parSequence_v2[F[_] : Traverse, A](container: F[IO[A]]): IO[F[A]] =
    container.parTraverse(x => x)
  ...
  override def run: IO[Unit] = parallelSingleIO_v3.map(_.sum).debug.void
