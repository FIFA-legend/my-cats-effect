...
object CyclicBarriers extends IOApp.Simple {
  ...
  /*
    ...
    A cyclic barrier will (semantically) block all fibers calling its await() method until wa have exactly N fibers waiting,
      at which point the barrier will unblock all fibers and reset to its original state.
    And further fiber will again block until we have exactly N fibers waiting.
    ...
    And so on.
  */
  ...
  def createUser(id: Int, barrier: CBarrier): IO[Unit] = for {
    ...
    _ <- IO(s"[user $id] Just heard there's a new social network - singing up for the waitlist...").debug
