...
import cats.effect.{Async, IO, IOApp, Sync, Temporal, Concurrent}
...
object PolymorphicAsync extends IOApp.Simple {
  ...
  val asyncMeaningOfLifeComplex: IO[Int] = IO.async { (cb: Callback[Int]) =>
    ...
    }.as(Some(IO("Cancelled!").debug.void)) // <-- finalizeer in case the computation gets cancelled
  }
  ...
  val asyncMeaningOfLife_v3 = asyncIO.evalOn(IO(42), myExecutionContext).guarantee(IO(threadPool.shutdown()))
   ...
   * 2 - tuple two effects with different requirments.
   */
